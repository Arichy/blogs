...
本文会介绍几个常见的测试类型, 以及在 Rust 里的相关 crates, 但不会过多介绍用途, 因为我都是让 AI 写的, 我自己也懒得去研究每个 crate 具体的用法.
...
通过各方面测试的保驾护航, 才能让我们安心地去迭代已有的功能, 开发效率会有数量级的提升. 并且, `cargo nextest r` 运行通过的舒适感比 `cargo build` 还强烈!
